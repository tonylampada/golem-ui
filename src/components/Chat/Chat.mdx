...
`Chat` is the conversation between a person and the agent: a message list that follows the newest
...
| Adapter | What Chat calls            | Why                                                                     |
| ------- | -------------------------- | ----------------------------------------------------------------------- |
| `Chat`  | `history()`                | The conversation as it stands on mount.                                 |
| `Chat`  | `subscribe(listener)`      | Every later change: a new message, and each token of a streaming reply. |
| `Chat`  | `send(text, attachments?)` | What the reader typed, with whatever they staged on the paperclip.      |

Chat takes no other adapter. It never fetches, never stores, and never reads a clock — a bubble's
...
```tsx
import { Chat, fakeChat } from 'golem-ui'
...
- **Invalid config.** Chat renders an error card instead of the conversation, in dev and in prod,
...
- **The last message is the reader's.** Chat reads that as a reply the agent owes and shows the
  thinking row until an agent message arrives. A history that _ends_ on a user message therefore
