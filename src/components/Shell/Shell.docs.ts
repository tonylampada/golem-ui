...
export const shellDocs: ComponentDocs = {
  ...
  tagline: 'The app frame: a chat column, the canvas beside it, and a top bar — tabs on a phone.',

  purpose: `\`Shell\` is the app frame: an agent chat column on one side, the canvas the agent builds into on the
...
the \`canvas\` slot). If you want a two-pane layout *inside* the canvas, that is not this component.`,

  adapters: [
    {
      adapter: 'Identity',
      calls: '`currentUser()`, `subscribe()`',
      why: "Shows the signed-in user's name in the top bar, and keeps it current when they sign out.",
    },
    {
      adapter: 'Navigation',
      calls: "`go('/')`",
  ...
  example: `import { Shell, fakeIdentity, fakeNavigation } from 'golem-ui'
