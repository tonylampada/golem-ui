...
`Auth` is everything a multi-user app needs around identity, so the app writes no login code. It is
...
| Adapter      | What Auth calls                                            | Why                                                                                   |
| ------------ | ---------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| `Identity`   | `currentUser()`, `subscribe()`                             | Who is here on mount, and every later flip. All three surfaces read it.               |
| `Identity`   | `signIn()`, `signOut()`                                    | The password mode, and the way out of both the account menu and the not-allowed card. |
| `Identity`   | `requestCode()`, `verifyCode()`                            | The `code` mode: the address first, the one-time code second.                         |
| `Identity`   | `signUp()`                                                 | Sign-up, carrying the invite token when the reader arrived on a link.                 |
| `Identity`   | `listMembers()`, `invite()`, `setRole()`, `removeMember()` | The member list and what a managing role does to it.                                  |
| `Navigation` | `current()`, `subscribe()`                                 | Reads `params.invite` — the app's router is what knows how its own URLs are shaped.   |

Auth takes no other adapter. It never fetches, never stores, and never reads `window`.
...
```tsx
import { Auth, Shell, fakeIdentity, fakeNavigation } from 'golem-ui'
...
- **Invalid config.** Auth renders an error card instead of the screen, in dev and in prod, naming
