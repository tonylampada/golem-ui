...
export const recordFormDocs: ComponentDocs = {
  name: 'RecordForm',
  slug: 'record-form',
  storybookPath: 'components-recordform',
  tagline: 'One record being written: a create form or an edit form, from one field list.',

  purpose: `\`RecordForm\` is one record being written: a create form or an edit form of the same collection
...
knows — \`{ key, label, type, format? }\` — and adds what makes a control out of it:`,
    {
      table: {
        headings: ['Key', 'On which types', 'What it does'],
        rows: [
          ['`required`', 'all', 'Blocks submit while empty, and marks the label'],
          [
            '`readOnly`',
            'all',
            'Shows the value formatted, with no control and no place in a patch',
          ],
          ...
          [
            '`options`',
            '`enum` (required)',
            '`[{ id, label }]` — the reader picks the label, the record gets the id',
          ],
          [
            '`min` / `max`',
            '`number` `money` `date` `datetime`',
            'The accepted range: a number, or an ISO string for a date',
          ],
          ['`multiline`', '`text`', 'A textarea, spanning both columns of `two-column`'],
          [
            '`users`',
            '`user`',
            '`members` is a select over `Identity.listMembers()`; `free` is a text box',
          ],
        ],
      },
    },
  ...
  adapters: [
    {
      ...
      why: 'Edit mode only, to fill the controls from the record.',
    },
    {
      adapter: 'Records',
      calls: '`create(collection, data)`',
      why: "Create mode's submit. Every editable field goes in.",
    },
    {
      adapter: 'Records',
    ...
    {
      adapter: 'Records',
      calls: '`remove(collection, id)`',
      why: 'The Delete button, after the confirm.',
    },
    {
      adapter: 'Identity',
      calls: '`listMembers()`',
      why: "Only for a `user` field with `users: 'members'`.",
    },
  ],

  adapterNotes: `RecordForm takes no other adapter. It never fetches, never routes and never reads a clock — a date
