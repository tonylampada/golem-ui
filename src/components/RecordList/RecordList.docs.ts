...
export const recordListDocs: ComponentDocs = {
  name: 'RecordList',
  slug: 'record-list',
  storybookPath: 'components-recordlist',
  tagline: 'Many records of one type: a table at desktop width, cards on a phone, from one config.',

  purpose: `\`RecordList\` is many records of one type on a screen: a table at desktop width, cards on a phone,
...
\`{ key, label, type, format?, primary? }\`:`,
    {
      table: {
        headings: ['`type`', 'Renders as', '`format` means'],
        rows: [
  ...
  adapters: [
    {
      ...
      why: 'Every page. The filter, the sort, the search text and the cursor go with it.',
    },
    {
      adapter: 'Records',
      calls: '`subscribe(collection)`',
      why: 'A row written by someone else. The list re-lists; it never polls.',
    },
  ],

  adapterNotes: `RecordList takes no other adapter. It never fetches, never routes and never reads a clock — a date
